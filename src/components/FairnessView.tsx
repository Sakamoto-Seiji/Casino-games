...
const FairnessView = ({ privateHash, publicSeed, Label, children }: any) => {
  ...
  return (
    <>
      <button onClick={() => setVerifyOpen(true)}>{children}</button>
      <Modal
        isOpen={verifyOpen}
        onClose={() => {
          setVerifyOpen(false);
        }}
        className="animate-zoomIn pb-0 bg-[#2d2d2d] max-w-[530px] w-svw rounded-md border-[2px] border-[#2fe42f]"
      >
        <div className=" flex-col">
          <div className="flex  items-center px-3 text-white">
            ...
            <div className="font-bold px-1">Fairness</div>
          </div>
          <div className="p-4">
            <SwitchTab
              disabled={false}
