...
const VerifyModal = ({ gameId, Label, setGameId }: any) => {
  ...
  return (
    <Modal
      ...
      className=" animate-zoomIn bg-[#2d2d2d] max-w-[530px] w-svw rounded-md border-[2px] border-[#ffe81a]"
    >
      <div className="p-3">
        ...
        <SwitchTab
          options={["Result", "Leaderboard"]}
          onChange={setActiveVerifyTab}
          disabled={false}
          active={activeVerifyTab}
        />
        {activeVerifyTab === 0 && (
          <>
          ...
          <div className="flex flex-col">
            {activeVerifyTab === 0 ? (
              <>
                ...
                <CustomInput
                  disabled={true}
                  value={showGame.privateSeed}
                  type="string"
                  icon={
                    <button
                      onClick={() => copyToClipboard(showGame.privateSeed)}
                ...
                <CustomInput
                  disabled={true}
                  value={showGame.publicSeed}
                  type="string"
                  icon={
                    <button
                      onClick={() => copyToClipboard(showGame.publicSeed)}
                      className="px-1 py-2 w-full "
                    ></button>
                  }
                />
